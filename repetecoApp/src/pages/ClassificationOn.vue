      ...
      <div class="btnNotification" role="group">
        <userProfile :profile="profile"></userProfile>
          <div class="btn-group pull-right" role="group" aria-label="...">
            ...
            <button type="button" class="btn btn-default active" onclick="location.href='classificationOn'" data-transition="slide"><i class="fa fa-star"> </i></button>
            <button type="button" class="btn btn-default" onclick="location.href='/classificationOff'"><i class="fa fa-star-o"> </i></button>
          </div>          
      </div>      
    </div> 
    <ul v-if="this.users.length != 0" class="list-group" id="searchUL">
      <userOn v-for="user in users" v-if="user.id_fb_users === profile.id" v-bind:key="user.id" v-bind:name="user.name" v-bind:imagem="user.imagem" v-bind:link="user.link" v-bind:friends="user.friends"></userOn>
    </ul>
